...
export function CoinFlipLeaderboard() {
        ...
        try {
          ...
          if (userData.type !== 'none' && userData.value) {
            ...
            leaderboardEntries.push({
              ...
              totalFlips: parseInt(String(value['total-flips']?.value || value.totalFlips?.value || '0')),
              ...
              headsWins: parseInt(String(value['heads-wins']?.value || value.headsWins?.value || '0')),
              tailsWins: parseInt(String(value['tails-wins']?.value || value.tailsWins?.value || '0')),
            });
          }
        } catch (err) {
          continue;
    ...
    <div className="bg-white rounded-lg shadow-md border border-gray-200">
      ...
      <div className="divide-y divide-gray-200">
        {entries.map((entry, index) => {
          const winRate = entry.totalFlips > 0 ? ((entry.wins / entry.totalFlips) * 100).toFixed(1) : '0.0';
          return (
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      {entry.totalFlips} jogadas • {entry.wins} vitórias ({winRate}%)
                    </p>
                    <p className="text-xs text-gray-400">
                      👤 {entry.headsWins} • 🪙 {entry.tailsWins}
                    </p>
                  </div>
                </div>
