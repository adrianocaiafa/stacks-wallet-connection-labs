...
export function DiceStats() {
  ...
  const fetchStats = async () => {
    ...
    try {
      ...
      const [totalRollsResult, userCountResult] = await Promise.all([
      ...
      const totalRollsData = cvToJSON(totalRollsResult);
      const userCountData = cvToJSON(userCountResult);

      setStats({
        totalRolls: parseInt(String(totalRollsData.value || '0')),
    ...
    <div className="bg-white rounded-lg shadow-md p-6 border border-gray-200">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="text-center">
          <p className="text-3xl font-bold text-blue-600">{stats.totalRolls}</p>
          <p className="text-sm text-gray-600">Total de Rolagens</p>
        </div>
        <div className="text-center">
